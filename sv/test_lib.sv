...
// Performance Stat Test
// Tests back-to-back write transactions with performance statistics
// Parameters:
//   - LEN: random inside [1:16] beats per transaction
//   - SIZE: max_width (4 bytes for 32-bit data)
//   - BURST: INCR
//   - Start address: aligned to 4 bytes
//   - Number of transactions: 5000
//   - Slave ready signals: always high
class axi4_performance_stat_test extends axi4_base_test;
    `uvm_component_utils(axi4_performance_stat_test)

    // Constructor
    function new(string name = "axi4_performance_stat_test", uvm_component parent = null);
        super.new(name, parent);
    endfunction

    // Run phase
    task run_phase(uvm_phase phase);
        axi4_performance_stat_sequence seq;

        phase.raise_objection(this);

        `uvm_info(get_type_name(), "===========================================", UVM_NONE)
        `uvm_info(get_type_name(), "       AXI4 PERFORMANCE STAT TEST", UVM_NONE)
        `uvm_info(get_type_name(), "===========================================", UVM_NONE)
        `uvm_info(get_type_name(), "Test Configuration:", UVM_NONE)
        `uvm_info(get_type_name(), "  - Burst Length: random [1:16] beats", UVM_NONE)
        `uvm_info(get_type_name(), "  - Transfer Size: 4 bytes (SIZE=2)", UVM_NONE)
        ...
        `uvm_info(get_type_name(), "  - Start Address: 0x1000_0000 (aligned)", UVM_NONE)
        `uvm_info(get_type_name(), "  - Number of Transactions: 5000", UVM_NONE)
        `uvm_info(get_type_name(), "  - Slave Ready: ALWAYS HIGH", UVM_NONE)
        `uvm_info(get_type_name(), "===========================================", UVM_NONE)

        // Create and configure the sequence
        seq = axi4_performance_stat_sequence::type_id::create("seq");
        ...
        if (!seq.randomize() with {
            m_size       == 2;            // 4 bytes per beat
            m_start_addr == 32'h1000_0000; // Aligned start address
            m_num_trans  == 5000;          // 5000 transactions
            m_burst      == INCR;         // INCR burst type
        }) begin
            `uvm_error(get_type_name(), "Sequence randomization failed")
        end
        ...
        `uvm_info(get_type_name(), "Performance stat test sequence completed", UVM_MEDIUM)
